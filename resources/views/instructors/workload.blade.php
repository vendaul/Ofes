...
<div class="page-header">
    <h1><i class="fas fa-eye"></i> Workload for
        {{ optional($instructor)->fname ?? 'Unknown' }}
        {{ optional($instructor)->lname ?? '' }}
    </h1>
...
<div class="card">
    <div class="card-header text-white bg-primary d-flex justify-content-between align-items-center flex-wrap gap-2">
        ...
        <div class="d-flex flex-wrap gap-2">
            <a href="{{ route('assignments.create', ['instructor_id' => $instructor->id, 'college_id' => $instructor->college, 'area_code' => $instructor->areacode]) }}" class="btn btn-light btn-sm">
            ...
            {{-- <button type="button" class="btn btn-success btn-sm" data-bs-toggle="modal" data-bs-target="#importStudentsModal">
            ...
            <a href="{{ route('instructors.index') }}" class="btn btn-outline-light btn-sm">
    ...
    <div class="table-responsive">
        <table class="table table-hover mb-0">
            ...
            <tbody>
                ...
                <tr>
                    <td>
                        <span class="badge bg-primary text-white">{{ $a->subject ? $a->subject->code : 'N/A' }}</span>
                    </td>
                    <td>
                        {{ $a->subject ? $a->subject->name : 'Unknown Subject' }}
                    </td>
                    <td>
                        <a href="{{ route('class-schedule-students.index', $a->id) }}" class="text-decoration-none">
                            <span class="badge bg-success">{{ $a->section ? $a->section->name : 'Unknown Section' }}</span>
                        </a>
                    </td>
                    <td>
                        <span class="badge bg-primary">{{ $a->ay }}</span>
                    </td>
                    <td>
                        <span class="badge bg-warning text-dark">{{ $a->term }}</span>
                    </td>
                    <td>
                        <div class="action-buttons">
                            <a href="{{ route('class-schedules.edit', $a->id) }}" class="btn btn-sm btn-warning me-1">
                                <i class="fas fa-edit"></i> Edit
                            </a>
                            <form method="POST" action="{{ route('class-schedules.destroy', $a->id) }}" style="display: inline;">
                                @csrf
                                @method('DELETE')
...
<div class="modal fade" id="importWorkloadModal" tabindex="-1" aria-labelledby="importWorkloadModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content">
            <form action="{{ route('instructors.workload.import', $instructor->id) }}" method="POST" enctype="multipart/form-data" id="importWorkloadForm">
                @csrf
                ...
                <div class="modal-body">
                    ...
                    <div class="row g-2">
                        <div class="col-md-6">
                            ...
                            <input type="text" id="import_school_year" name="school_year" class="form-control" value="{{ old('school_year', $activePeriod->year ?? '') }}" placeholder="e.g. 2025-2026" required>
                        </div>
                        <div class="col-md-6">
                            <label for="import_semester" class="form-label">Semester / Term</label>
                            <input type="text" id="import_semester" name="semester" class="form-control" value="{{ old('semester', $activePeriod->term ?? '') }}" placeholder="e.g. First" required>
                        </div>
                    </div>
                    ...
                    <div class="row g-2 mt-1">
                        <div class="col-md-6">
                            ...
                            <select id="import_area_code" name="area_code" class="form-select" required>
                                ...
                                <option value="{{ $code }}" {{ (string) old('area_code', $defaultAreaCode) === (string) $code ? 'selected' : '' }}>{{ $name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-6">
                            ...
                            <select id="import_college_id" name="college_id" class="form-select" required>
                                ...
                                <option value="{{ $college->id }}" data-area-code="{{ $college->area_code }}" {{ (string) old('college_id', $defaultCollegeId) === (string) $college->id ? 'selected' : '' }}>{{ $college->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="row g-2 mt-1">
                        <div class="col-md-6">
                            ...
                            <select id="import_program_id" name="program_id" class="form-select">
                                ...
                                <option value="{{ $program->id }}" {{ (string) old('program_id', $defaultProgramId ?? '') === (string) $program->id ? 'selected' : '' }}>{{ $program->course_program ?: $program->code }} - {{ $program->name ?: 'N/A' }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-6">
                            ...
                            <select id="import_curriculum_id" name="curriculum_id" class="form-select">
                                ...
                                <option value="{{ $curriculum->id }}" data-course-id="{{ $curriculum->course_id }}" {{ (string) old('curriculum_id', $defaultCurriculumId ?? '') === (string) $curriculum->id ? 'selected' : '' }}>{{ $curriculum->code }} - {{ $curriculum->desc ?: 'N/A' }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <datalist id="sections-list">
                            @foreach(($instructorSections ?? collect()) as $section)
