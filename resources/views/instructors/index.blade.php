...
<div class="card">
    <div class="card-header text-white bg-primary d-flex justify-content-between align-items-center flex-wrap gap-2">
        ...
        <div class="d-flex flex-wrap gap-2">
            ...
            <a id="assignSubjectBtn" href="{{ route('assignments.create') }}" class="btn btn-outline-light btn-sm">
    ...
    <div class="table-responsive instructors-table-wrap">
        <table class="table table-hover mb-0">
            ...
            <tbody>
                ...
                <tr>
                    <td>
                        <span class="badge bg-primary text-white">{{ $instructor->empid }}</span>
                    </td>
                    <td>
                        <strong>{{ $instructor->fname }} {{ $instructor->lname }}</strong>
                    </td>
                    <td>
                        <span class="badge bg-info text-white">{{ $instructor->academic_rank ?? 'N/A' }}</span>
                    </td>
                    <td>{{ $instructor->position ?? 'N/A' }}</td>
                    <td>{{ $instructor->collegeRelation ? $instructor->collegeRelation->name : 'N/A' }}</td>
                    <td>
                        <div class="action-buttons">
                            <a href="{{ route('instructors.workload', $instructor->id) }}" class="btn btn-sm btn-info">
                                <i class="fas fa-eye"></i> View
                            </a>
                            <a href="{{ route('instructors.edit', $instructor->id) }}" class="btn btn-sm btn-warning">
    ...
    <div class="card-body">
        <div class="alert alert-info mb-0">
            <i class="fas fa-info-circle"></i> No instructors found. <a href="{{ route('instructors.create') }}">Create one now</a>
        </div>
    </div>
    @endif
</div>
...
<div class="modal fade" id="importInstructorWorkloadModal" tabindex="-1" aria-labelledby="importInstructorWorkloadModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content">
            <form method="POST" action="{{ route('instructors.importWithWorkload') }}" enctype="multipart/form-data" id="importInstructorWorkloadForm">
                @csrf
                ...
                <div class="modal-body">
                    ...
                    <input type="hidden" name="area_code" id="import_area_code" value="{{ old('area_code', $selectedArea ?? '') }}">
                    <input type="hidden" name="college_id" id="import_college_id" value="{{ old('college_id', $selectedCollege ?? '') }}">

                    <div class="row g-2">
                        <div class="col-md-6">
                            <label class="form-label">Area</label>
                            <input type="text" class="form-control" value="{{ $areaOptions[$selectedArea] ?? ($selectedArea ?? '') }}" readonly>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">College</label>
                            <input type="text" class="form-control" value="{{ $selectedCollegeName ?? '' }}" readonly>
                        </div>
                    </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (addInstructorBtn) {
            addInstructorBtn.addEventListener('click', function() {
                ...
                window.location.href = '{{ route("instructors.create") }}?area_code=' + encodeURIComponent(area) + '&college_id=' + encodeURIComponent(college);
            });
        }
