        ...
        <form id="academicFilterForm" method="GET" class="d-flex gap-2 align-items-end">
            ...
            <div class="form-group mb-0">
                ...
                <select id="area_code" name="area_code" class="form-select" onchange="submitAcademicFilter(this.form)">
                    ...
                    <option value="{{ $code }}" {{ $code == $selectedArea ? 'selected' : '' }}>{{ $name }}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group mb-0">
                ...
                <select id="college_id" name="college_id" class="form-select" onchange="submitAcademicFilter(this.form)">
                    ...
                    <option value="{{ $id }}" {{ $id == $selectedCollege ? 'selected' : '' }}>{{ $name }}</option>
                    @endforeach
                </select>
            </div>
    ...
    <div class="card">
        <div class="card-body">
            <ul class="nav nav-tabs mb-4" id="subjectSectionTabs" role="tablist">
                <li class="nav-item">
                    <button class="nav-link {{ $activeTab === 'curriculum' ? 'active' : '' }}" id="curriculum-tab" data-bs-toggle="tab" data-bs-target="#curriculum" type="button">
                        <i class="fas fa-sitemap"></i> Program & Curriculum
                    </button>
                </li>
                <li class="nav-item">
                    <button class="nav-link {{ $activeTab === 'subjects' ? 'active' : '' }}" id="subjects-tab" data-bs-toggle="tab" data-bs-target="#subjects" type="button">
            ...
            <div class="tab-content">
                <div class="tab-pane fade {{ $activeTab === 'curriculum' ? 'show active' : '' }}" id="curriculum">
                    ...
                    <div class="collapse mb-3" id="createProgramPanel">
                        <div class="card border-primary shadow-sm">
                            ...
                            <div class="card-body">
                                <form method="POST" action="{{ route('subjects.programs.store') }}" class="row g-2 align-items-end">
                                    @csrf
                                    <div class="col-md-2">
                                        ...
                                        <select id="program_area" name="area_code" class="form-select" required>
                                            <option value="">Select Area</option>
                                            @foreach($areaOptions as $code => $name)
                                            <option value="{{ $code }}" {{ (string) $selectedArea === (string) $code ? 'selected' : '' }}>{{ $name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="col-md-3">
                                        ...
                                        <select id="program_college" name="college_id" class="form-select" required>
                                            <option value="">Select College</option>
                                            @foreach($collegeOptions as $id => $name)
                                            <option value="{{ $id }}" {{ (string) $selectedCollege === (string) $id ? 'selected' : '' }}>{{ $name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="col-md-2">
                    ...
                    <div class="collapse mb-3" id="createCurriculumPanel">
                        <div class="card border-success shadow-sm">
                            ...
                            <div class="card-body">
                                <form method="POST" action="{{ route('subjects.curriculums.store') }}" class="row g-2 align-items-end">
                                    @csrf
                                    <div class="col-md-3">
                                    ...
                                    <div class="col-md-4">
                                        ...
                                        <select id="curr_program" name="course_id" class="form-select" required>
                                            ...
                                            <option value="{{ $program->id }}">{{ $program->course_program ?: $program->code }} - {{ $program->name ?: 'N/A' }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="col-md-2">
                            ...
                            <div class="table-responsive">
                                <table class="table table-bordered table-hover align-middle mb-0">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            <td><span class="fw-semibold">{{ $program->course_program ?: $program->code ?: '-' }}</span></td>
                                            <td>{{ $program->name ?: '-' }}</td>
                                            <td>{{ $areaOptions[$program->area_code] ?? ($program->area_code ?: '-') }}</td>
                                            <td>{{ $collegeOptions[$program->college_id] ?? ($program->college_id ?: '-') }}</td>
                                            <td>
                                                <div class="d-flex gap-1 flex-wrap">
                                                    <button type="button" class="btn btn-sm btn-warning program-edit-btn" data-bs-toggle="modal" data-bs-target="#editProgramModal" data-update-url="{{ route('subjects.programs.update', ['id' => $program->id], false) }}" data-program-id="{{ $program->id }}" data-program-area-code="{{ $program->area_code }}" data-program-college-id="{{ $program->college_id }}" data-program-code="{{ $program->course_program ?: $program->code }}" data-program-name="{{ $program->name }}">
                                                        <i class="fas fa-edit"></i> Edit
                                                    </button>
                                                    <form method="POST" action="{{ route('subjects.programs.destroy', $program->id) }}" onsubmit="return confirm('Delete this program?');">
                                                        @csrf
                                                        @method('DELETE')
                        ...
                        <div class="card-body p-0">
                            <div class="table-responsive">
                                <table class="table table-bordered table-hover align-middle mb-0">
                                    ...
                                    <tbody>
                                        @forelse($curriculumsByProgramId as $programId => $programCurriculums)
                                        @php
                                        ...
                                        @foreach($programCurriculums as $index => $curriculum)
                                        <tr>
                                            @if($index === 0)
                                            <td rowspan="{{ $rowspan }}" class="align-top bg-light">
                                                @if($program)
                                                <div class="fw-semibold">{{ $program->course_program ?: $program->code }}</div>
                                                <div class="text-muted small">{{ $program->name ?: '-' }}</div>
                                                @else
                                                <div class="fw-semibold">Program ID: {{ $programId ?: 'N/A' }}</div>
                                            ...
                                            <td>
                                                <span class="fw-semibold">{{ $curriculum->code }}</span>
                                                @if((int)($activeCurriculumId ?? 0) === (int)$curriculum->id)
                                                <span class="badge bg-success ms-2">Active</span>
                                                @endif
                                            </td>
                                            <td>
                                                <div class="d-flex gap-1 flex-wrap">
                                                    <form method="POST" action="{{ route('subjects.curriculums.set-active', $curriculum->id) }}">
                                                        @csrf
                                                        <button type="submit" class="btn btn-sm {{ (int)($activeCurriculumId ?? 0) === (int)$curriculum->id ? 'btn-success' : 'btn-outline-success' }}">
                                                            <i class="fas fa-check-circle"></i> Set Active
                                                        </button>
                                                    </form>
                                                    <button class="btn btn-sm btn-warning curriculum-edit-btn" type="button" data-bs-toggle="modal" data-bs-target="#editCurriculumModal" data-update-url="{{ route('subjects.curriculums.update', ['id' => $curriculum->id], false) }}" data-curriculum-id="{{ $curriculum->id }}" data-area-code="{{ $curriculum->area_code }}" data-college-id="{{ $curriculum->college_id }}" data-course-id="{{ $curriculum->course_id }}" data-curriculum-code="{{ $curriculum->code }}">
                                                        <i class="fas fa-edit"></i> Edit
                                                    </button>
                                                    <form method="POST" action="{{ route('subjects.curriculums.destroy', $curriculum->id) }}" onsubmit="return confirm('Delete this curriculum?');">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit" class="btn btn-sm btn-danger"><i class="fas fa-trash"></i> Delete</button>
                                                    </form>
            ...
            <div class="tab-pane fade {{ $activeTab === 'subjects' ? 'show active' : '' }}" id="subjects">
                <div class="mb-3 d-flex justify-content-between align-items-center">
                    <div class="d-flex gap-2 align-items-center">
                        <a href="{{ route('subjects.create', ['tab' => 'subjects', 'area_code' => $selectedArea, 'college_id' => $selectedCollege]) }}" id="addSubjectBtn" class="btn btn-primary">
                    ...
                    <form method="GET" action="{{ route('subjects.index') }}" class="input-group w-50">
                        ...
                        <input id="subjectSearch" type="text" name="subject_search" class="form-control" placeholder="Search all subjects by name or code..." value="{{ $searchTerm }}">
                        <button id="subjectSearchBtn" type="button" class="btn btn-outline-secondary"><i class="fas fa-search"></i> Search</button>
                        @if($searchTerm)
                        <a href="{{ route('subjects.index', ['tab' => 'subjects', 'area_code' => $selectedArea, 'college_id' => $selectedCollege]) }}" class="btn btn-outline-danger" title="Clear search"><i class="fas fa-times"></i></a>
                        @endif
                    </form>
                </div>
                @if($searchTerm)
                <div class="alert alert-info py-2 mb-2">
                    <i class="fas fa-search"></i> Showing all subjects matching <strong>"{{ $searchTerm }}"</strong> across all areas and colleges.
                    <a href="{{ route('subjects.index', ['tab' => 'subjects', 'area_code' => $selectedArea, 'college_id' => $selectedCollege]) }}" class="ms-2 alert-link">Clear search</a>
                </div>
                @endif

                @if($subjects->count() > 0)
                <form method="POST" action="{{ route('subjects.destroyMany') }}" id="bulkDeleteSubjectsForm" class="mb-2">
                    @csrf
                    @method('DELETE')
                    <input type="hidden" name="tab" value="subjects">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td class="text-center">
                                    <input type="checkbox" class="subject-select-item" name="subject_ids[]" value="{{ $subject->id }}" form="bulkDeleteSubjectsForm">
                                </td>
                                <td>{{ $subject->firstCurriculumSubject?->curriculum?->course?->code ?? $subject->curricula->first()?->course?->code ?? '-' }}</td>
                                <td>{{ $subject->firstCurriculumSubject?->curriculum?->code ?? $subject->curricula->first()?->code ?? '-' }}</td>
                                <td>{{ $subject->subject_year ?: '-' }}</td>
                                <td>{{ $subject->subject_term ?: '-' }}</td>
                                <td><span class="badge bg-primary text-white">{{ $subject->id }}</span></td>
                                <td><code>{{ $subject->code }}</code></td>
                                <td><strong>{{ $subject->name }}</strong></td>
                                <td>
                                    <div class="d-flex gap-1 flex-wrap">
                                        <a href="{{ route('subjects.edit', $subject->id) }}?tab=subjects&area_code={{ $selectedArea }}&college_id={{ $selectedCollege }}" class="btn btn-sm btn-warning">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <form action="{{ route('subjects.destroy', $subject->id) }}?tab=subjects" method="POST">
                                            @csrf
                                            @method('DELETE')
                ...
                <div class="alert alert-info">
                    ...
                    <a href="{{ route('subjects.create', ['tab' => 'subjects', 'area_code' => $selectedArea, 'college_id' => $selectedCollege]) }}">Create one now</a>
                    @else
...
<div class="modal fade" id="importSubjectModal" tabindex="-1" aria-labelledby="importSubjectModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <form method="POST" action="{{ route('subjects.import') }}" enctype="multipart/form-data" id="importSubjectForm">
                @csrf
                <input type="hidden" name="tab" value="subjects">
                    ...
                    <div class="row g-2 mt-3">
                        <div class="col-md-6">
                            ...
                            <select id="import_area_code" name="area_code" class="form-select" required>
                                <option value="">Select Area</option>
                                @foreach($areaOptions as $code => $name)
                                <option value="{{ $code }}" {{ (string) $importDefaultAreaCode === (string) $code ? 'selected' : '' }}>{{ $name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-6">
                            ...
                            <select id="import_college_id" name="college_id" class="form-select" required>
                                ...
                                <option value="{{ $college->id }}" data-area-code="{{ $college->area_code }}" {{ (string) $importDefaultCollegeId === (string) $college->id ? 'selected' : '' }}>{{ $college->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="row g-2 mt-2">
                        <div class="col-md-6">
                            ...
                            <select id="import_program_id" name="import_program_id" class="form-select">
                                <option value="">Select Program</option>
                                @foreach($programs as $program)
                                <option value="{{ $program->id }}" {{ (int)($importDefaultProgramId ?? 0) === (int)$program->id ? 'selected' : '' }}>{{ $program->course_program ?: $program->code }} - {{ $program->name ?: 'N/A' }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-6">
                            ...
                            <select id="import_curriculum_id" name="import_curriculum_id" class="form-select">
                                ...
                                <option value="{{ $curriculum->id }}" data-course-id="{{ $curriculum->course_id }}" {{ (int)($importDefaultCurriculumId ?? 0) === (int)$curriculum->id ? 'selected' : '' }}>{{ $curriculum->code }} - {{ $curriculum->desc ?: 'N/A' }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <select id="edit_curriculum_program" name="course_id" class="form-select" required>
                                @foreach($programs as $optProgram)
                                <option value="{{ $optProgram->id }}">{{ $optProgram->course_program ?: $optProgram->code }} - {{ $optProgram->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-6">
