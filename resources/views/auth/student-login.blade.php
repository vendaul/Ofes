...
<body>
    <div class="login-container">
        ...
        <form method="POST" action="{{ route('student.login.submit') }}" id="student-login-form">
            @csrf

            <div class="mb-3">
                <label for="sid" class="form-label">Student ID</label>
                <div class="input-group">
                    <input type="text" id="sid" name="sid" class="form-control" placeholder="Enter student id" value="{{ old('sid') }}" required>
    ...
    <script>
        document.getElementById('check-student').addEventListener('click', function() {
            ...
            fetch(`{{ url('/student/check') }}/${encodeURIComponent(sid)}`)
                ...
                .then(data => {
                    ...
                    if (!data.email) {
                        const registerUrl = '{{ route("student.register.form") }}?sid=' + encodeURIComponent(sid);
                        statusText.innerHTML = 'Student found. Email not assigned yet. <a href="' + registerUrl + '">Complete registration</a> or use <a href="{{ route("student.login") }}">Student Login</a> after registration.';
                        emailField.style.display = 'none';
                        passwordField.style.display = 'none';
                        loginBtn.disabled = true;
