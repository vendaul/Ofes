    ...
    <div class="row justify-content-center">
        <div class="col-lg-10 col-xl-8">
            <div class="card shadow">
                ...
                <div class="card-body">
                    <form action="{{ route('evaluate.store') }}" method="POST" id="evaluationForm">
                        @csrf
                        <input type="hidden" name="class_schedule_id" value="{{ $class_schedule_id ?? $assign_id ?? '' }}">
                                    ...
                                    <div class="rating-section">
                                        ...
                                        <div class="rating-scale d-flex align-items-center">
                                            <div class="rating-options d-flex gap-2 flex-wrap">
                                                @for ($i = 1; $i <= 5; $i++) <div class="rating-option">
                                                    <input type="radio" name="ratings[{{ $question->question_id }}]" value="{{ $i }}" id="rating_{{ $question->question_id }}_{{ $i }}" class="btn-check" required>
                                                    <label class="btn btn-outline-primary rating-btn" for="rating_{{ $question->question_id }}_{{ $i }}">
                                                        {{ $i }}
                                                    </label>
                                            </div>
                                            @endfor
                ...
                <div class="text-center mt-4 d-flex flex-column align-items-center gap-2">
                    <div class="d-flex gap-3">
                        <a href="{{ route('student.dashboard') }}" class="btn btn-secondary btn-lg px-5" onclick="return confirm('Are you sure you want to cancel? Your answers will not be saved.')">
