...
<div class="card mb-4">
    ...
    <div class="card-body">
        <form action="{{ route('settings.semester.update') }}" method="POST" class="row g-3">
            @csrf
            <div class="col-md-4">
                <label for="year" class="form-label">School Year</label>
                <input id="year" name="year" value="{{ old('year') }}" type="text" class="form-control @error('year') is-invalid @enderror" placeholder="2024-2025">
                @error('year')
            ...
            <div class="col-md-4">
                ...
                <input id="term" name="term" value="{{ old('term') }}" type="text" class="form-control @error('term') is-invalid @enderror" placeholder="First">
                @error('term')
                <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            <div class="col-md-4">
                ...
                <input id="name" name="name" value="{{ old('name') }}" type="text" class="form-control" placeholder="First Term 2024-2025">
            </div>
            <div class="col-12">
                <button class="btn btn-primary" type="submit"><i class="fas fa-save"></i>
                    {{ !empty($editPeriod) ? 'Update Period' : 'Save Period' }}
                </button>
                @if(!empty($editPeriod))
                <a href="{{ route('settings.semester') }}" class="btn btn-secondary">Cancel Edit</a>
                @endif
            </div>
        </form>
    </div>
...
<div class="card">
    ...
    <div class="card-body table-responsive">
        @if($periods->count() > 0)
        <form method="POST" action="{{ route('settings.semester.update') }}" id="bulkDeletePeriodsForm" class="mb-3">
            @csrf
            ...
            <tbody>
                ...
                <tr>
                    <td class="text-center">
                        <input type="checkbox" class="period-select-item" name="bulk_delete_period_ids[]" value="{{ $period->id }}" form="bulkDeletePeriodsForm">
                    </td>
                    <td>{{ $period->id }}</td>
                    <td>{{ $period->year ?? '-' }}</td>
                    <td>{{ $period->term ?? '-' }}</td>
                    <td>{{ $period->name ?? '-' }}</td>
                    <td>
                        @if(!empty($activePeriod) && $activePeriod->id == $period->id)
                    ...
                    <td class="d-flex gap-2">
                        @if(empty($activePeriod) || $activePeriod->id != $period->id)
                        <form method="POST" action="{{ route('settings.semester.update') }}" style="display:inline;">
                            @csrf
                            <input type="hidden" name="set_active_id" value="{{ $period->id }}" />
                            <button type="submit" class="btn btn-primary btn-sm"><i class="fas fa-star"></i> Set Active</button>
                        </form>
                        @endif
                        <button type="button" class="btn btn-warning btn-sm edit-period-btn" data-id="{{ $period->id }}" data-year="{{ $period->year }}" data-term="{{ $period->term }}" data-name="{{ $period->name }}">
                            <i class="fas fa-edit"></i> Edit
                        </button>
                        <form method="POST" action="{{ route('settings.semester.update') }}" onsubmit="return confirm('Delete this period?');" style="display:inline;">
                            @csrf
...
<div class="modal fade" id="editPeriodModal" tabindex="-1" aria-labelledby="editPeriodModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form id="editPeriodForm" action="{{ route('settings.semester.update') }}" method="POST">
                @csrf
